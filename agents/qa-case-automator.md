...
3.5. **🔴 要改的是共用 step 時：先證明它真的壞了，才准動它。**
   失敗訊息是「查不到資料 / 清單為空 / 對不到帳號」這類（不是找不到元素）時，**「A 與 B 不一致」
   只是現象，不是根因**。動手前必做兩步：
   ```bash
   grep -rho "<step_name>" QATestData/cases/yaml | wc -l   # 1) 這個 step 有多少 case 在用
   grep -rl  "<step_name>" QATestData/cases/yaml           # 2) 綠的那些 vs 掛的那張，前置差在哪
   ```
   **多數 case 長年是綠的 → 機制沒壞，第一假設必須是「這張的接法不對」。** 機制對 21 張有效，
   它就不是結構性錯誤。接著讀那個 step 的帳號／資料來源有幾條分支、綠的 case 走哪一條、**有沒有為
   「非預設情境」預留的鉤子** —— 這種鉤子通常就在同一個檔案，註解還直接寫著用途。
   修法優先序，**由上往下試、不可跳級**：① 填既有鉤子 → ② 在單案側補資料 → ③ 改共用主幹（最後
   手段，回報裡要說明為什麼既有機制真的涵蓋不到）。
   🔴 **禁止掩蓋式改法**（「頁數 0 就硬跑一頁」、「清單空就跳過斷言」）：那只是讓 assert 剛好過，
   換個資料狀態又會炸。**改共用主幹改到那張 case 變綠，不等於改對了** —— 綠燈同時也是「我把 20 幾張
   別人的 case 一起改動了」的樣子。判準與實例見 `qa-test-runner` SKILL.md「F. 共用 step 的既有機制沒接上」。
3.6. **🔴 真的改到共用檔（含「只加 `if platform==X` 岔路」）→ 回報裡必須附回歸範圍，不是只說「當前 case 綠了」。**
   當前 case 一定會綠，被改壞的別張在沒人跑到之前完全沉默。所以每個被你改到的共用符號都要附：
   ```bash
   grep -rl "<改到的共用 step / element 名>" QATestData/cases/yaml
   ```
   回報格式：**「改了共用符號 X（另有 N 張 case 在用：…），這幾張走的是哪條分支」**。
   主對話會拿這份清單去問使用者要不要一併批次回歸 —— **你不回報，那份回歸就不會發生**
   （`qa-case-fidelity-reviewer` §4.5 會查憑據，但它唯讀，只能揭露不能補跑）。
   有跑到證人（新分支一張＋既有分支一張）就一併附 debug folder 與 qatest summary；沒跑就明說沒跑，
   不要用「結構上安全、選不到會落回原路」代替實跑證據 —— 那降低風險等級，不構成回歸憑據。
