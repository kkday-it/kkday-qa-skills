...
4. **🔴 動手寫 automation code 前，先 `Skill(qa-automation-writer)` 載入規範並遵守——不是「記得才用」，是硬前置。** Page Object / Test Step / API / case data 一律照它。
...
8. **單一父用的子步驟收成「父 function 內的巢狀 `def`」，不要攤成 top-level `_helper`**：closure 讀父層 `pages`/`uidriver`（不需 `@function_recorder`），照 repo 既有慣例（如 `verify_add_product_into_wish_list_playwright` 內含 `clean_text`/`add_into_wish_list`…）。理由：一堆 top-level `_`-prefixed helper 會污染 module，且**它們不是 yaml 能呼叫的 step**（yaml 只對 top-level test step function）。**只有跨多個 test step 真正共用的** helper 才留 top-level `@function_recorder()`。判準：單一父用→內嵌；跨 step 共用→top-level decorated。
