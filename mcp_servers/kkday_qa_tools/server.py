...
# ── QA 自動化：共享 locator 記憶 ─────────────────────────────────────────


@mcp.tool()
def get_verified_locator(
    flow: str = "",
    page: str = "",
    platform: str = "web",
    env: str = "stage",
    ...
    """從共享 locator registry 取回『已驗過的候選 selector』當 QA 自動化定位元素的起手 hints。

    讓任何接了本 MCP 的 AI（不只裝了 qa-automation-writer skill 的 Claude Code）都能取用
    跨人累積的 locator 共享記憶，把重挖從「盲找」變「驗證 + 微調」。

    ⚠️ 重要：回傳的 selectors 是**候選 hint、不是真理**。用前必須在當前 DOM 逐一驗證
    selector 存不存在，驗過才用、驗不過標 stale 並回退「從零挖」。後端只是跨人共享/趨勢層，
    不保證候選仍有效（前端改 class 就會腐爛）。**且不同環境（sit0x / sit20x / stage）、
    甚至 web 與 mweb，部署的前端不見得是同一版，跨環境/跨平台的候選只能『參考』，不可照搬**。

    Args:
        flow: 流程/區域 key（如 things-to-do-search），一次批次取整組相關元素。flow / page 擇一必填。
        page: 頁面語意 key（如 things-to-do-landing）。
        platform: web | mweb（預設 web）。web 與 mweb 是不同部署，不可互相套用。
        env: 環境代號 stage / sit0x / sit20x（如 sit04 / sit206；預設 stage）。**現階段禁 prod**，
             用 server 既有 _check_env 判斷擋下（非法或 prod 會 raise）。
        component: 元件語意 key（可選，縮小範圍）。
    """
    if not (flow or page):
        return {
            "ok": False,
            "error": "需至少提供 flow 或 page 其一",
            "entries": [],
            "methodology": [],
        }
    # 環境判斷用 server 既有的 _check_env（唯一真源）：接受 stage / sit0x / sit20x，
    # 擋掉 prod 與空值。現階段禁打 prod —— _check_env 本就不接受 prod，非法會 raise。
    _check_env(env)
    params = {
        k: v
        for k, v in {
    ...
    # 不走 _call：避免 _check_env 誤擋 env=prod（locator 的 env 是前端站別，非商品環境）；
    # 改自己打 GET 並手動埋點。
    path = "/api/qa-automation/locator-registry"
    _start = time.monotonic()
    try:
        resp = requests.get(
            f"{BASE}{path}", headers=_headers(), params=params, timeout=(10, 30)
        )
        if not resp.ok:
            raise RuntimeError(f"GET {path} → {resp.status_code}: {resp.text[:300]}")
        result = resp.json()
        _emit_analytics(
            "get_verified_locator",
            params,
            result,
            None,
            (time.monotonic() - _start) * 1000,
    ...
    if isinstance(result, dict):
        result["usage_warning"] = (
            "候選 hint 非真理：用前務必在當前 DOM 逐一驗證 selector，驗過才用、"
            "驗不過標 stale 並回退從零挖。後端只是共享/趨勢層。"
        )
    return result
