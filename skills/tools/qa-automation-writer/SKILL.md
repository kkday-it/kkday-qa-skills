...
不准為了消滅 magic number 而抽出**只是複述該數值**的常數：

```python
# ❌ 禁止：常數名沒有比字面值多講任何事
_WECHAT_GREEN_MAX_RED = 90
_ALLOW_BUTTON_MIN_ROW_COVERAGE = 0.25
_AUTHORIZATION_PAGE_POLL_ROUNDS = 7
_AUTHORIZATION_PAGE_POLL_SECONDS = 3
```

`_WECHAT_GREEN_MAX_RED = 90` 讀完只知道「最大紅色是 90」——那是 `red < 90` 已經講完的事。它把
一行變三行、把讀者從邏輯處推到檔頭再推回來，**資訊量零**。這類重構是為了討好 AI reviewer 的
評分項而製造的噪音，不是可維護性。

判準（要抽常數必須至少滿足一項，否則直接寫字面值）：

| 可以抽 | 為什麼 |
|---|---|
| 同一個值在**多處**出現，會一起改 | 抽出來才有單一修改點 |
| 值本身**帶外部意義**、光看數字讀不出來 | 例：`_IOS_APP_STATE_FOREGROUND = 4`（4 是 Appium 定義的列舉值） |
| 值是**一組相關設定**、要並排比較才看得懂 | 例：一組跨平台的比例參數 |

⚠️ **AI reviewer 常把這條反過來要求**（「建議把 `0.25`、`0.5`、`7`、`3` 抽成具名常數」）。
